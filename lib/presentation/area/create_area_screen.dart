...
class CreateAreaScreen extends StatefulWidget {
  const CreateAreaScreen({super.key, this.areaTemplate});
...
class _CreateAreaScreenState extends State<CreateAreaScreen> {
  ...
  @override
  void initState() {
    if (widget.areaTemplate?.title != null) {
      nameController.text = widget.areaTemplate!.title;
    }

    if (widget.areaTemplate?.subtitle != null) {
      descController.text = widget.areaTemplate!.subtitle;
    }

    selectedIcon = widget.areaTemplate?.icon ?? Icons.home;
    selectedColor = widget.areaTemplate?.color ?? Color(0xFF7AA2FF);
  ...
  void _createFolder(BuildContext context) {
    ...
    context.read<AreaProvider>().createArea(context, folder);
  }
}
